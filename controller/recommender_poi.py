...
class RecommenderRestaurant():
    ...
    def recommend(self, lat, lng, review, tags):
        ...
        # cluster_tags = self.recommender_tags.predict(np.array([tags]).reshape(1, -1))[0]
        cluster_review = 0
        cluster_tags = None
    ...
    def __recommend_restaurants(self, cluster_location, cluster_review, cluster_tags, lat, lng):
        ...
        results = db.execute_query(
            f'SELECT title, address, type, rating, reviews as review, latitude, longitude, price, description, tags, ( 2 * asin(sqrt(cos(radians({lat})) * cos(radians(r.latitude)) * pow(sin(radians(({lng} - r.longitude) / 2)), 2) + pow(sin(radians(({lat} - r.latitude) / 2)), 2))) *6371) AS distance FROM restaurants r WHERE  cluster_location = \'{cluster_location}\' AND cluster_rate = \'{cluster_review}\' ORDER BY distance ASC LIMIT {self.MAX_RESULTS}')
